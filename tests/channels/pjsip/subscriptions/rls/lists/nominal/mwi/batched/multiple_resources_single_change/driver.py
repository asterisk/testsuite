...
class TestDriver(object):
    ...
    def on_subscription_state_change(self):
        ...
        if self.state == ESTABLISHED:
            ...
            if interval < 5.0:
                ...
                self.fail_test()
            LOGGER.debug("State change to {0}".format(ONE_MESSAGE))
            self.state = ONE_MESSAGE
        if self.state == REFRESHED:
            interval = time.time() - self.bob_time
            if time.time() - self.bob_time < 5.0:
                LOGGER.error("Interval {0} too brief".format(interval))
                self.fail_test()
            LOGGER.debug("State change to {0}".format(TWO_MESSAGES))
            self.state = TWO_MESSAGES

    def on_subscription_refreshed(self):
        if self.state != ONE_MESSAGE:
            LOGGER.error("Unexpected state change from {0}. Expected state "
                         "{1}".format(self.state, ONE_MESSAGE))
            self.fail_test()
    ...
    def on_subscription_terminated(self):
        if self.state != TWO_MESSAGES:
            LOGGER.error("Unexpected state change from {0}. Expected state "
                         "{1}".format(self.state, TWO_MESSAGES))
            self.fail_test()
