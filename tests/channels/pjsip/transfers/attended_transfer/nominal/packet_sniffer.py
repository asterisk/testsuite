...
class Sniffer(VOIPListener):
    ...
    def add_call(self, packet):
        ...
        LOGGER.debug("%s: %s" % (callid, self.get_call_state(callid)))

    def is_call_known(self, callid):
        """Check if call is known

        Keyword Arguments:
        callid String of the call-id to update

        Returns:
        True if call is known. False otherwise.
        """
        for endpoint, endpoint_calls in self.calls.iteritems():
            if [call for call in endpoint_calls if callid in call]:
                return True
        return False
    ...
    def update_call_state(self, callid, newstate):
        ...
        LOGGER.debug("%s: %s" % (callid, newstate))
    ...
    def check_invite(self, packet):
        ...
        if not self.is_call_known(callid):
        ...
        if packet.transport_layer.header.source != 5060:
            ...
            LOGGER.warn("Unexpected INVITE found: %s" % callid)
            return
    ...
    def check_200(self, packet):
        ...
        if self.get_call_state(callid) == "CONNECTING":
            ...
            LOGGER.debug("Endpoints in remote RTP bridge: %s" %
                    reinvited_endpoints)
    ...
    def send_user_event(self, user_event, event_data):
        ...
        """
        LOGGER.debug("Sending UserEvent '%s'" % user_event)
...
def hold(test_object, triggered_by, source, event):
    ...
    try:
        ...
        if "PJ_EINVALIDOP" in err.err_msg():
            reactor.callLater(.25, phone.hold_call)
        else:
